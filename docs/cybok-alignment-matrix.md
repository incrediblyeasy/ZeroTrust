...
| Service-to-service authentication | api-gateway + protected-service | The protected service fails closed and requires the gateway-injected `INTERNAL_GATEWAY_SECRET` on every request, so a direct call bypassing Envoy/OPA is refused. A CA-signed mTLS PKI is also generated for the transport-layer upgrade. | `services/protected-service/main.py` — gateway-auth middleware; `scripts/generate-certs.sh`; `envoy/certs/` |
...
| Log integrity / tamper detection | Logstash keyed hash chaining | Each log entry's hash = HMAC-SHA256(`AUDIT_HMAC_KEY`, previous_hash + current_body); chain starts with "GENESIS" sentinel; the secret key prevents an attacker from recomputing a consistent chain after tampering; standalone verifier script provided | `elk/logstash/pipeline.conf` — Ruby filter; `scripts/verify_log_chain.py` |
...
| CyBOK sub-principle | Framework component | Implementation | Evidence |
|---|---|---|---|
| Network isolation & transport security | Docker network + Envoy | Internal services run on an isolated `ztac-net` bridge; all host ports are bound to loopback (`127.0.0.1`). A CA-signed mTLS PKI is provisioned (`envoy/certs/`) as the production upgrade for encrypted, mutually-authenticated transport. | `docker-compose.yml`; `envoy/certs/` |
...
| Defence in depth | All components | Multiple layers: JWT validation (RS256, issuer, azp allowlist) at the gateway, ABAC policy at OPA, deny-by-default, app-layer gateway-auth at the resource, body-size limits, brute-force protection, and keyed-HMAC audit integrity | Full test suite (`tests/`) |
